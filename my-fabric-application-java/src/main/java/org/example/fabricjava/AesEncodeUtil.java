...
public class AesEncodeUtil {
    ...
    public static String encrypt(String cleartext) {
        ...
        try {
            ...
            //ASE_KEY1 = GatewayConfig.getPrivateKey()
    ...
    public static String encrypt1(String cleartext, String pkId) {
        ...
        try {
            ...
            IvParameterSpec zeroIv = new IvParameterSpec(VIPARA.getBytes());
            //两个参数，第一个为私钥字节数组， 第二个为加密方式 AES或者DES
            SecretKeySpec key = new SecretKeySpec(pkId.getBytes(), "AES");
            //实例化加密类，参数为加密方式，要写全
            Cipher cipher = Cipher.getInstance("AES/CBC/PKCS5Padding"); //PKCS5Padding比PKCS7Padding效率高，PKCS7Padding可支持IOS加解密
            //初始化，此方法可以采用三种方式，按加密算法要求来添加。（1）无第三个参数（2）第三个参数为SecureRandom random = new SecureRandom();中random对象，随机数。(AES不可采用这种方法)（3）采用此代码中的IVParameterSpec
            cipher.init(Cipher.ENCRYPT_MODE, key, zeroIv);
    ...
    public static String decrypt1(String encrypted, String skId) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        String content = "http://data.zjzwfw.gov.cn/interface/gateway.do";
        // 加密
        System.out.println("加密前：" + content);
        String encryptResult = encrypt(content);
        String jiamihou = "guhVvXtR10SDIS14GNhA1gS/ZT/gN8m99UcQt2CWPqCa02IdImnEQddcvD0I2X3r";
        System.out.println("加密后：" + new String(encryptResult));
        // 解密
        String decryptResult = decrypt(encryptResult);
        String decryptResult1 = decrypt(jiamihou);
